...
var g = (s, t, i) => t in s ? l(s, t, { enumerable: !0, configurable: !0, writable: !0, value: i }) : s[t] = i;
var c = (s, t, i) => (g(s, typeof t != "symbol" ? t + "" : t, i), i);
import { css as d, addClass as p } from "fourdom";
var n = /* @__PURE__ */ ((s) => (s.containerClassName = "starscore-container", s.scoreWrapperClassName = "starscore-radio-group", s.scoreItemClassName = "starscore-radio", s.scoreIconClassName = "starscore-radio__icon", s.scoreVoidIconClassName = "starscore-radio__void-icon", s))(n || {});
...
class f {
  ...
  setValue(t) {
    return t < 0 ? this.value = 0 : t > this.options.count ? this.value = this.options.count : this.value = t, this.render(), this;
  }
  getValue() {
    return this.value;
  }
  ...
  get scoreItems() {
    const t = [], i = this.renderValue;
    for (let e = 1; e <= this.options.count; e++) {
      const r = e - i, o = i >= e ? 1 : r >= 1 ? 0 : Number.parseFloat((1 - r).toFixed(1));
      t.push({
        score: e,
        active: i >= e,
        proportion: o,
  ...
  emitChange(t) {
    this.value !== t && (this.options.onChange && this.options.onChange(t), this.setValue(t));
  }
  scoreValueFormat(t) {
    if (this.options.disabled || this.options.readonly)
      return null;
    const i = t.target, e = t.offsetX, r = this.getScoreItemFromChild(i);
    if (r) {
      let o = Number(r.dataset.score);
      return this.options.allowHalf && e <= o / 2 && (o = o - 0.5), this.options.clearable && this.value === o ? 0 : o;
    }
    return null;
  }
  clickListener(t) {
    const i = this.scoreValueFormat(t);
    i !== null && this.emitChange(i);
  }
  registerListeners() {
  ...
  initCSSVars() {
    d(this.getContainer(), {
      "--starscore-size": a(this.options.size),
      ...
      "--starscore-gutter": a(this.options.gutter)
    });
  }
  generateRadioHTML(t) {
    return `
  ...
  generateRadioGroupHTML() {
    const t = this.scoreItems.reduce((r, o) => (r += `${this.generateRadioHTML(o)}`, r), ""), i = this.options.disabled ? "disabled" : "", e = this.options.readonly ? "readonly" : "";
    return `
      <div class="${n.scoreWrapperClassName} ${i} ${e}">${t}</div>
    `;
  }
  render() {
    const t = this.getContainer(), i = `${this.generateRadioGroupHTML()}`;
    p(t, n.containerClassName), t.innerHTML = i, this.registerListeners();
  }
  destory() {
