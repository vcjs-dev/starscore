...
var d = (s, t, i) => t in s ? g(s, t, { enumerable: !0, configurable: !0, writable: !0, value: i }) : s[t] = i;
var c = (s, t, i) => (d(s, typeof t != "symbol" ? t + "" : t, i), i);
import { css as p, addClass as v } from "fourdom";
...
class C {
  ...
  get scoreItems() {
    ...
    for (let i = 1; i <= this.options.count; i++) {
      const r = i - this.value, e = this.value >= i ? 1 : r >= 1 ? 0 : Number.parseFloat((1 - r).toFixed(1));
      t.push({
        score: i,
        active: this.value >= i,
        proportion: e,
  ...
  clickListener(t) {
    if (this.options.disabled || this.options.readonly)
      return;
    const i = t.target, r = t.offsetX, e = this.getScoreItemFromChild(i);
    if (e) {
      let o = Number(e.dataset.score);
      this.options.allowHalf && r <= o / 2 && (o = o - 0.5);
      const l = this.options.clearable && this.value === o ? 0 : o;
      this.emitChange(l);
    }
  }
  registerListeners() {
  ...
  generateRadioGroupHTML() {
    const t = this.scoreItems.reduce((e, o) => (e += `${this.generateRadioHTML(o)}`, e), ""), i = this.options.disabled ? "disabled" : "", r = this.options.readonly ? "readonly" : "";
    return `
      <div class="${n.scoreWrapperClassName} ${i} ${r}">${t}</div>
    `;
  }
  render() {
    const t = this.getContainer(), i = `${this.generateRadioGroupHTML()}`;
    v(t, n.containerClassName), t.innerHTML = i, this.registerListeners();
  }
  destory() {
